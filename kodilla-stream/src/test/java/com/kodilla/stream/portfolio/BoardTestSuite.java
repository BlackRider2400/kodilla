...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        double average = 55.0 / 3.0;

        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));

        //When
        List<Integer> daysCount = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .map(task -> (int) ChronoUnit.DAYS.between(task.getCreated(), task.getDeadline()))
                .collect(toList());

        double resultAverageCount = IntStream.range(0, daysCount.size())
                .map(n -> daysCount.get(n))
                .average().getAsDouble();

        //Then
        assertEquals(average, resultAverageCount);
